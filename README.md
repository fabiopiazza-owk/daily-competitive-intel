...
Automated daily PM intelligence briefing for K Platform.
...
2. **Synthesizes with Claude** (`claude-opus-4-6`) using a PM-specific system prompt that:
   ...
   - Surfaces regulatory signals affecting enterprise deals
   - Outputs 3 concrete build signals for the backlog

3. **Commits the report** as `reports/YYYY-MM-DD.md` to this repo
4. **Posts a Slack summary** via Workflow Builder with top signals + build items
...
```bash
git clone https://github.com/your-org/daily-competitive-intel
cd daily-competitive-intel
```
...
| `SLACK_WEBHOOK_URL` | Optional | Slack Workflow Builder webhook URL |
...
5. **Partnership & Ecosystem Signals** — ecosystem developments
...
│   ├── notify_slack.py            # Slack Workflow Builder notification
│   └── requirements.txt
├── reports/
